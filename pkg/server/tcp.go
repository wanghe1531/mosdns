		...
		go func() {
			...
			for {
				...
				go func() {
					...
					meta := QueryMeta{ClientAddr: clientAddr, ServerName: serverName}
					if real, ok := ExtractClientIP(req, clientAddr); ok {
						meta.RealClientAddr = real
					}
					r := h.Handle(tcpConnCtx, req, meta, pool.PackTCPBuffer)
					if r == nil {
